...
The R code below first loads **rstan** and **pcFactorStan**. The next command that follows gives **rstan** permission to use all of the available CPU cores to run multiple chains in parallel.
...
The nonzero count of divergent transitions means that these items contained too little signal to estimate. We could try again with `varCorrection=1.0`, but we are going to exclude them instead.
The model succeeded on the rest of the items.
I requested `iter=1000L` to demonstrate how `calibrateItems` will resample the model until the `n_eff` is large enough and the `Rhat` is small enough. Items _skill_, _predict_, and _reward_ needed 1500 samples instead of the default 1000.
