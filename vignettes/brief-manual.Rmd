...
our analyses.
I fit model 'factor_ll' instead of 'factor' so I can use the
**loo** package to look for outliers.
We also need to take care that the data `pafp` matches
the data fit by the model so we can map back from the model
back to the data. Hence, we use `prepCleanData` and
update `pafp` using the usual the data cleaning sequence
of `filterGraph` and `normalizeData`.

```{r factorData, cache=TRUE}
...
examine the parameters of interest. In particular, we do not
care about `log_lik` because these are the per-observation
likelihoods for **loo**.
...
fit3 <- pcStan("factor_ll", data=dl, include=FALSE, iter=4000L,
               pars=c('rawUnique', 'rawUniqueTheta', 'rawFactor', 'rawLoadings'))
```
...
There are no observations above the customary threshold of 0.5,
so we pick a lower threshold for the purpose of demonstration.

```{r factorLoo, cache=TRUE}
options(mc.cores=1)  # otherwise loo consumes too much RAM
l1 <- toLoo(fit3)
ot <- outlierTable(dl, l1, 0.45)
```
```{r, results='hide'}
print(ot)
```
...
Why is it unexpected that pilates requires much more skill than climbing?
We can examine how other participants have responded to this item,

```{r}
pafp[pafp$pa1 == ot[1,'pa1'] & pafp$pa2 == ot[1,'pa2'],
     c('pa1','pa2','skill')]
```

Hm, only one participant compared climbing and pilates.
Maybe this subject had not seen the Alex Honnold documentary Free Solo (2018)?
Let us look a little deeper to understand why this response
was unexpected.

```{r}
loc <- sapply(ot[1,c('pa1','pa2','item')], unfactor)
summary(fit3, pars=paste0("theta[",loc[paste0('pa',1:2)],
                          ",", loc['item'],"]"))$summary
```

Here we find that, based on comparisons with other activities,
climbing was estimated to require about 1.5 units of skill
more than pilates.
We could continue our investigation by looking at which
responses justify these skill estimates.
...
| sigma     | lognormal(1,1) | relative item scale                  |
...
|----------------|---------------------|--------------------------------------|
| threshold      | normal(0,2)         | item response thresholds             |
| unique         | normal(1,1) T[0,]   | scale of unique scores               |
