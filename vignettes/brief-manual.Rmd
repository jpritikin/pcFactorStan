...
```{r}
sims <- extract(fit1, permuted = FALSE, inc_warmup=FALSE)
...
Again, everything looks good. Time for a plot,
...
I am inclined to flip the sign so that _walking_ has a negative skill
and _golf_ has a positive skill, but the sign is not important for
this brief tutorial. Actually the main reason that we fit this model
is to find a scaling factor that would produce a standard
...
| factorProp     | _see below_         | signed factor variance proportion    |

Thresholds for all items are combined into a single vector.
`factorProp` is derived using Equation 3 of Gelman et al. (in press).
Therefore, the prior for `factorProp` is some function of the priors
