...
indeed _equal_ then this can result in good model fit, but the
practical interpretation is that the item is useless.
...
the Item _chatter_ should be excluded as well.
...
Note that the marginal posterior standard deviations of `alpha` are all less than 0.25.
...
To check the fit diagnostics, we have to take care only to
examine the parameters of interest. The factor model outputs
...
```{r responseCurves2, cache=TRUE}
df <- responseCurve(dl, fit3,
  responseNames=c("much more","somewhat more", 'equal',
                  "somewhat less", "much less"))
ggplot(df) +
  geom_line(aes(x=worthDiff,y=prob,color=response,linetype=response,
...
The maximum $\widehat R$ is too high or the effective sample size is too low.
We need to try again with more iterations,

```{r factor2, message=FALSE, results='hide', cache=TRUE}
fit3 <- pcStan("factor_ll", data=dl, include=FALSE, iter=4000L, chains=6L,
               pars=c('rawUnique', 'rawUniqueTheta', 'rawFactor', 'rawLoadings'))
```
...
that a few lines are shown.

```{r}
xx <- which(ot[,'pa1'] == 'tennis' & ot[,'pa2'] == 'water skiing' & ot[,'pick'] == -2)
```
...
is from `r exam[2,'2.5%']` to `r exam[2,'98%']`.
...
And there you have it. If you have not already,
