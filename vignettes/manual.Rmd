...
We can take a look at the item response curves.
They look similar to the corresponding curves
from the correlation model.

```{r responseCurvesF, cache=TRUE}
...
ric <- summary(fit3, pars=c('residualItemCor'), prob=c(.1,.5,.9))$summary
rl <- matrix(ric[,'10%'], length(dl$nameInfo$item), length(dl$nameInfo$item),
       dimnames=list(dl$nameInfo$item,dl$nameInfo$item))
r50 <- matrix(ric[,'50%'], length(dl$nameInfo$item), length(dl$nameInfo$item),
              dimnames=list(dl$nameInfo$item,dl$nameInfo$item))
ru <- matrix(ric[,'90%'], length(dl$nameInfo$item), length(dl$nameInfo$item),
       dimnames=list(dl$nameInfo$item,dl$nameInfo$item))
ric5 <- fivenum(r50[lower.tri(r50)])
mask <- sign(rl) == -sign(ru) # small, not statistically significant
r50[mask] <- 0
...
Median residual correlations ranged from `r ric5[1]` to `r ric5[5]`.
To reduce visual clutter,
small correlations with 80\% posterior intervals crossing zero were hidden.
This is conservative; almost none of the correlations would remain
if a 90\% interval was used instead, indicating adequate model fit.
_Stakes_ is an ideal item, least correlated with other items.
These faint residual correlations are hard to interpret.
It seems likely that any survey measure is going to exhibit
...
standard normally distributed because scale is arbitrary.
...
`factor` scores are standard normally distributed. `rawLoadings` are
