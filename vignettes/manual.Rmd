...
if (sum(pareto_k_values(l1)>.5) != 0) stop("Outliers?!")
```

There are no conspicuous outliers. We can look at `p_loo`,
...
```{r}
xx <- which(ot[,'pa1'] == 'calisthenics' & ot[,'pa2'] == 'sex' & ot[,'item'] == 'skill' & ot[,'pick'] == 2)
```
...
The last response category is _much less_.
Putting it all together we obtain an endorsement of
_`r ot[xx,'pa1']` involves much less skill than `r ot[xx,'pa2']`_.
...
`r ot[xx,'pa2']` was estimated to involve `r exam[2,'mean'] - exam[1,'mean']` units more
skill than `r ot[xx,'pa1']`. However, note the wide posterior quantiles!
...
responses justified these _skill_ estimates.
