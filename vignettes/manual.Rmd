...
small correlations with 60\% posterior intervals crossing zero were set to zero.
This is conservative; Almost none of the correlations would remain
if a 90\% interval was used instead.
_Stakes_ is an ideal item, uncorrelated with other items.
There seem to be two main clusters.
_Body_, _reward_, and _chatter_ seem more emotional
while _novelty_, _creative_, and _present_ seem more cognitive.
That the items faintly cluster along a cognitive-emotional
spectrum, with _skill_ in between, is not necessarily problematic.
...
|-----------|-------------------|--------------------------|
| threshold | lognormal(0,1)    | item response thresholds |
...
|-----------------|---------------------|--------------------------------------|
| threshold       | lognormal(0,1)      | item response thresholds             |
| pathLoadings    | normal(0,5)         | signed scale of factor scores        |
...
| residualItemCor | N/A                 | residual item correlation matrix     |
...
`rawPathProp`, `rawNegateFactor`, `rawSeenFactor`, and `residual` from sampling.
